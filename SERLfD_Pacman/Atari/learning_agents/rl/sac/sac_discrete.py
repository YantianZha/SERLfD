...
from learning_agents.architectures.cnn import Conv2d_MLP_Categorical, Conv2d_MLP_Model
...
from learning_agents.architectures.mlp import MLP, FlattenMLP, TanhGaussianDistParams, CategoricalDistParams
...
class SAC_Discrete_Agent(Agent):
    """ Discrete SAC agent interacting with environment.
    Attrtibutes:
    ...
    def __init__(self, env, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=None):
        ...
        target_entropy = -np.log((1.0 / self.action_dim)) * self.hyper_params.target_entropy_ratio
    ...
    def _set_to_default_MLP(self):
        # create actor
        self.actor = CategoricalDistParams(
    ...
    def _set_to_default_CNN(self):
        # create actor
        self.actor = Conv2d_MLP_Categorical(input_channels=self.state_channel,
        ...
        # create double q_critic
        # qf 1
        self.qf1 = Conv2d_MLP_Model(input_channels=self.state_channel,
                                    fc_input_size=self.network_cfg.fc_input_size,
                                    fc_output_size=self.action_dim,
                                    nonlinearity=self.network_cfg.nonlinearity,
                                    channels=self.network_cfg.channels,
                                    ...
                                    strides=self.network_cfg.strides,
                                    paddings=self.network_cfg.paddings,
                                    fc_hidden_sizes=self.network_cfg.fc_hidden_sizes_qf,
                                    fc_hidden_activation=self.network_cfg.fc_hidden_activation).to(device)
        # target qf 1
        self.target_qf1 = Conv2d_MLP_Model(input_channels=self.state_channel,
                                    fc_input_size=self.network_cfg.fc_input_size,
                                    fc_output_size=self.action_dim,
                                    nonlinearity=self.network_cfg.nonlinearity,
                                    channels=self.network_cfg.channels,
        ...
        self.target_qf1.load_state_dict(self.qf1.state_dict())
        # qf 2
        self.qf2 = Conv2d_MLP_Model(input_channels=self.state_channel,
                                    fc_input_size=self.network_cfg.fc_input_size,
                                    fc_output_size=self.action_dim,
                                    nonlinearity=self.network_cfg.nonlinearity,
                                    channels=self.network_cfg.channels,
                                    ...
                                    fc_hidden_activation=self.network_cfg.fc_hidden_activation).to(device)
        # target qf 2
        self.target_qf2 = Conv2d_MLP_Model(input_channels=self.state_channel,
                                    fc_input_size=self.network_cfg.fc_input_size,
                                    fc_output_size=self.action_dim,
                                    nonlinearity=self.network_cfg.nonlinearity,
                                    channels=self.network_cfg.channels,
    ...
    def select_action(self, state):
        ...
        if self.total_step < self.hyper_params.init_random_actions and not self.testing:
            selected_action = np.array(self.env.action_space.sample())
        else:
            state = self._preprocess_state(state)
            deterministic = True if self.testing else self.hyper_params.deterministic_action
            self.actor.eval()
            with torch.no_grad():
    ...
    def _get_qf_loss(self, obs, actions, rewards, next_obs, dones, alpha, gamma, reward_clip=None):
        if reward_clip is not None:
            rewards = torch.clamp(rewards, min=reward_clip[0], max=reward_clip[1])
        masks = 1 - dones

        with torch.no_grad():
            _, _, next_log_probs, _ = self.actor(next_obs)
            next_action_probs = next_log_probs.exp()

            soft_next_targets = torch.min(
                self.target_qf1(next_obs),
                self.target_qf2(next_obs),
            ) - alpha * next_log_probs
            soft_v_targets = next_action_probs * soft_next_targets

            soft_v_targets = soft_v_targets.mean(dim=-1, keepdim=True)
            soft_q_targets = self.hyper_params.reward_scale * rewards + gamma * masks * soft_v_targets

        q1_values = self.qf1(obs)
        q1_prediction = q1_values.gather(1, actions.long())
        q2_values = self.qf2(obs)
        q2_prediction = q2_values.gather(1, actions.long())
        ...
        return q1_loss_element_wise, q2_loss_element_wise, q1_values, q2_values
    ...
    def update_model(self):
        ...
        actor_loss = alpha * log_probs - new_actions_q_values
        actor_loss = (curr_action_probs * actor_loss).mean()
