...
from learning_agents.rl.dqn.dq_fd import DQfDAgent

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


class SQLfD_Discrete_Agent(DQfDAgent):
    """
    Soft DQfD Agent. (Here we assume that we are using stacked frames and grayscaled observation)
    Attribute:
    ...
    def __init__(self, env, args, log_cfg, hyper_params, network_cfg, optim_cfg,
                 demo_preprocessor, encoder=None, logger=None):
        """Initialize."""
        DQfDAgent.__init__(self, env, args, log_cfg, hyper_params, network_cfg, optim_cfg,
                           demo_preprocessor, encoder=encoder, logger=logger)
