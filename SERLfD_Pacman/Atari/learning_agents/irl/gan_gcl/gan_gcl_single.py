...
from utils.trajectory_utils import read_expert_demo, get_indexed_trajs, demo_discrete_actions_to_one_hot, \
    extract_experiences_from_indexed_trajs
...
class GAN_GCL_Single(GAIL):
    ...
    def __init__(self, env, policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=None):
        ...
        """
        GAIL.__init__(self, env, policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=encoder, logger=logger)
    ...
    def _set_to_default_CNN(self):
        self.irl_model = DefaultDiscriminatorCNN(
    ...
    def get_reward(self, state, action=None):
        ...
        """
        # energy is -log(p_tau) = -log(1/Z*exp(-cost(tau))) or -log(1/Z*exp(reward))= -reward
    ...
    def update_irl(self, indexed_sampled_trajs):
        ...
        for it in range(self.hyper_params.num_iteration_update):
            ...
            if self.hyper_params.traj_batch_size < 0:
                ...
                sample_states, sample_actions, _, _, _ = self.sampled_traj_buf.get_experiences_from_trajs(
            ...
            for i in range(arrange.shape[0] // batch_size):
                ...
                # nest list/flat list:  torch.Tensor([energy, energy, energy, energy, ...])
                energy = self.irl_model(torch.cat((batch_states, batch_actions), 1))
                # shape of log_p_tau (energy is in the shape of [[value], [value], [value], ...]
                log_p_tau = torch.sum(-energy, dim=1, keepdim=True)

                log_q_tau = batch_log_probs.unsqueeze(1)
                # print('log p tau: ', log_p_tau[[1, 2, 50, 51]])
                # print('log q tau: ', log_q_tau[[1, 2, 50, 51]])
