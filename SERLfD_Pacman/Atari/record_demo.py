...
explainer_model_path = '/Users/lguan/Desktop/model/explainer_182870_0.tar'
policy_model_path = '/Users/lguan/Desktop/model/dqfd_182870_0.tar'
...
def run_game():
    ...
    env = PytorchImage(eatGhostPacmanGymEnv.EatGhostPacmanGymEnv(config_file), is_to_gray=True, is_normalize=False, rescale=None, resize=[84, 84])
    ...
    policy_hyper_params = Dict(dict(gamma=0.99,
                                    ...
                                    # N-Step Buffer
                                    n_step=3,   # if n_step <= 1, use common replay buffer otherwise n_step replay buffer
                                    # Double Q-Learning
    ...
    sql_fd_policy = SEGL_SQLfD_Discrete_Policy(env, policy_args, policy_log_cfg,  policy_hyper_params, policy_network_cfg,
    ...
    network_cfg = Dict(dict(use_cnn=True,
                            ...
                            fc_hidden_sizes=[512, 512],
                            fc_hidden_activation=torch.relu))
    optim_cfg = Dict(dict(lr_explainer=3e-4, weight_decay=0))
