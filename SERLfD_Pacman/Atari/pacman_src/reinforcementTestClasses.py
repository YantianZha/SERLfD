...
from pacman_src import testClasses, gridworld, pacman
import sys, os
import time
from pacman_src.util import Experiences
...
class ValueIterationTest(testClasses.TestCase):
    ...
    def executeNIterations(self, grades, moduleDict, solutionDict, n, checkPolicy):
        ...
        for action in actions:
            ...
            if self.comparePrettyValues(qValues, solutionDict[qValuesKey]):
                ...
                fileOutString += outString
        if checkPolicy:
            if not self.comparePrettyValues(policyPretty, solutionDict['controller_policy']):
                testPass = False
                outString = "Policy is NOT correct.\n"
                outString += "   Student solution:\n %s\n" % self.prettyValueSolutionString('controller_policy', policyPretty)
                outString += "   Correct solution:\n %s\n" % self.prettyValueSolutionString('controller_policy', solutionDict['controller_policy'])
                stdOutString += outString
                fileOutString += outString
    ...
    def writeSolution(self, moduleDict, filePath):
        with open(filePath, 'w') as handle:
            ...
            handle.write(self.prettyValueSolutionString('controller_policy', policyPretty))
    ...
    def executeNExperiences(self, grades, moduleDict, solutionDict, n, checkValuesAndPolicy):
        ...
        if checkValuesAndPolicy:
            ...
            if not self.comparePrettyValues(policyPretty, solutionDict['controller_policy']):
                testPass = False
                outString = "Policy is NOT correct."
                outString += "   Student solution:\n\t%s" % self.prettyValueSolutionString('controller_policy', policyPretty)
                outString += "   Correct solution:\n\t%s" % self.prettyValueSolutionString('controller_policy', solutionDict['controller_policy'])
                stdOutString += outString
                fileOutString += outString
        return testPass, stdOutString, fileOutString
...
### q2/q3
### =====
## For each parameter setting, compute the optimal controller_policy, see if it satisfies some properties
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        #    _                  controller_policy choice not checked
        #    N, E, S, W controller_policy action must be north, east, south, west
        #
        self.policy = parseGrid(testDict['controller_policy'])
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not policyPassed:
            ...
            self.addMessage('    Student controller_policy at %s: %s' % (differPoint, policy[differPoint]))
            self.addMessage('    Correct controller_policy at %s: %s' % (differPoint, actionMap[self.policy[differPoint[0]][differPoint[1]]]))
            self.addMessage('    Student controller_policy:')
            self.printPolicy(policy, False)
            self.addMessage("        Legend:  N,S,E,W at states which move north etc, X at states which exit,")
            self.addMessage("                 . at states where the controller_policy is not defined (e.g. walls)")
            self.addMessage('    Correct controller_policy specification:')
            self.printPolicy(self.policy, True)
            self.addMessage("        Legend:  N,S,E,W for states in which the student controller_policy must move north etc,")
            self.addMessage("                 _ for states where it doesn't matter what the student controller_policy does.")
            self.printGridworld()
            return self.testFail(grades)
        ...
        if self.pathVisits != None and self.pathVisits not in path:
            ...
            self.printPolicy(policy, False)
            self.addMessage("        Legend:  N,S,E,W at states which move north etc, X at states which exit,")
            self.addMessage("                 . at states where controller_policy not defined")
            self.printGridworld()
            return self.testFail(grades)
    ...
    def printPolicy(self, policy, policyTypeIsGrid):
        ...
        #         self.addMessage('      (%s,%s) %s' % (state[0], state[1], controller_policy[state]))
