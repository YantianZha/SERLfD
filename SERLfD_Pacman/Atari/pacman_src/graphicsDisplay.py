...
from pacman_src.graphicsUtils import *
import math
import time
...
OBSERVATION_RETURN_TENSOR = False
...
class PacmanGraphics:
    ...
    def drawAgentObjects(self, state):
        ...
        for index, agent_state in enumerate(state.agentStates):
            if agent_state.isPacman:
                image = self.drawPacman(agent_state, index)
                self.agentImages.append((agent_state, image))
            else:
                image = self.drawGhost(agent_state, index)
                self.agentImages.append((agent_state, image))
        refresh()
    ...
    def getGhostColor(self, ghost, ghostIndex):
        if ghost.scaredTimer > 0 and IS_CHANGE_SCARED_COLOR:
            return SCARED_COLOR
        else:
    ...
    def getRgbObservation(self):
        ...
        """
        rgb_img = Image.open(io.BytesIO(getPostscript().encode('utf-8')))
        if OBSERVATION_RETURN_TENSOR:
    ...
    def removeGhost(self, state, ghostIndex):
        ...
        for ghost_image_part in ghost_image_parts:
            remove_from_screen(ghost_image_part)
...
POSTSCRIPT_OUTPUT_DIR = '/Users/lguan/Desktop/Temp Docs/frames'
FRAME_NUMBER = 0
import os
