...
def run_game():
    ...
    hyper_params = Dict(dict(buffer_size=int(1e3),  # size of (sampled/expert) trajectories buffer
                             ...
                             traj_batch_size=128,   # number of trajectories used to update discriminator per iteration (-1 means using all trajectories)
    ...
    gan_gcl = GAN_GCL_Single(env, trpo_policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=expr_logger)
    gan_gcl.train()
