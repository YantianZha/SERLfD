...
from learning_agents.controller_policy.ppo_policy import PPOPolicy
...
def run_game():
    ...
    policy_hyper_params = Dict(dict(gamma=0.99,
                                    ...
                                    clip_epsilon=0.2,  # clipping epsilon for PPO, default: 0.2
                                    policy_clip_grad=40,
                                    ...
                                    batch_size=64,  # number of state-action pairs in mini-batch
                                    initial_random_action=int(0),
                                    num_critic_update=1,  # number of critic update per update
                                    num_policy_update=1,  # number of controller_policy update per update
    ...
    ppo_policy = PPOPolicy(env, policy_args, policy_log_cfg, policy_hyper_params,
    ...
    hyper_params = Dict(dict(buffer_size=int(1e3),  # size of (sampled/expert) trajectories buffer
                             ...
                             traj_batch_size=24,    # number of trajectories used to update discriminator per iteration (-1 means using all trajectories)
    ...
    network_cfg = Dict(dict(use_cnn=False, hidden_sizes_irl=[100, 100]))
    optim_cfg = Dict(dict(lr_discrim=3e-4, weight_decay=1e-4))

    gail_agent = GAIL(env, ppo_policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=expr_logger)
    gail_agent.train()
