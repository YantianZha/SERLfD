...
from learning_agents.irl.airl.airl_single import AIRL_Single
...
experiment_name = 'gail-trpo-lunar-continuous'
...
def run_game():
    ...
    policy_hyper_params = Dict(dict(gamma=0.99,
                                    lamda=0.98,
                                    buffer_size=int(1e1),      # not used in irl algorithms
                                    is_fusion=True,  # whether also use past samples to update irl (otherwise only use new sampled traj)
                                    traj_batch_size=5,  # number of trajectories used to update discriminator per iteration (-1 means using all trajectories)
                                    batch_size=64,       # number of state-action paris used to update controller_policy
                                    initial_random_action=int(0),   # must be zero if used as controller_policy in irl
                                    num_iteration_update=2,  # multiple learning updates per iteration
                                    clip_param=0.2,  # clipping parameter for PPO (default: 0.2)
                                    ))
    policy_network_cfg = Dict(dict(hidden_sizes_actor=[100, 100],
                                   hidden_sizes_vf=[100, 100],
                                   hidden_sizes_qf=[100, 100]))
    ...
    args = parse_args()
    log_cfg = Dict()
    hyper_params = Dict(dict(gamma=0.98,
                             buffer_size=int(1e3),  # size of (sampled/expert) trajectories buffer
                             is_fusion=True,    # whether also use past samples to update irl (otherwise only use new sampled traj)
                             n_samples_epoch=2048,      # minimum number of sampled state-action pairs per epoch
                             traj_batch_size=15,    # number of trajectories used to update discriminator per iteration (-1 means using all trajectories)
                             traj_fixed_length=80,  # the fixed length of trajectories
                             batch_size=64,     # number of state-action pairs used to update discriminator per inner-iteration
                             num_iteration_update=2,  # number of iteration per update
                             max_traj_length=1000,  # maximum length of a single rollout
                             ))
    network_cfg = Dict(dict(hidden_sizes_reward=[100, 100],
                            hidden_size_value=[100, 100]
                            ))
    optim_cfg = Dict(dict(lr_reward_fn=3e-4,
                          lr_value_fn=3e-4,
                          weight_decay=1e-4))

    gail_agent = AIRL_Single(env, trpo_policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=expr_logger)
    gail_agent.train()
