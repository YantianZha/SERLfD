# run_segl_single_dqfd_predicate.py
# ------------------
...
from learning_agents.controller_policy.segl_dqfd_predicate_policy import SEGL_DQfD_Predicate_Policy
...
def run_game():
    ...
    policy_hyper_params = Dict(dict(gamma=0.99,
                                    ...
                                    # LfD Setting
                                    margin=0.8,
                                    pretrain_step=int(1000),  # the paper use 1,000,000 mini-batch updates
                                    ...
                                    # Feature Augmenting Setting
                                    use_flatten_input=False,
                                    ))
    policy_network_cfg = Dict(dict(fc_input_size=3141,
                                   ...
                                   x_shape=[4, 84, 84],    # size of the input
                                   ))
    policy_optim_cfg = Dict(dict(lr_dqn=1e-5,
    ...
    dqfd_policy = SEGL_DQfD_Predicate_Policy(env, policy_args, policy_log_cfg,  policy_hyper_params, policy_network_cfg,
    ...
    network_cfg = Dict(dict(use_cnn=True,
                            ...
                            fc_hidden_sizes=[512, 512],
                            fc_hidden_activation=torch.relu))
    optim_cfg = Dict(dict(lr_explainer=1e-4, weight_decay=1e-5))
    expr_logger.save_config_wandb(config={"irl_hyper_params": hyper_params,
                                          "irl_network_cfg": network_cfg,
                                          "irl_optim_cfg": optim_cfg})

    segl_agent = SEGL_Single_Update_Step(env, dqfd_policy, args, log_cfg, hyper_params, network_cfg,
