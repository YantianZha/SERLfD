...
def run_game():
    ...
    hyper_params = Dict(dict(buffer_size=int(1e3),  # size of (sampled/expert) trajectories buffer
                             ...
                             is_fusion=True,
                             # whether also use past samples to update irl (otherwise only use new sampled traj)
                             traj_batch_size=128,
                             # number of trajectories used to update discriminator per iteration (-1 means using all trajectories)
                             traj_fixed_length=32,  # the fixed length of trajectories
                             batch_size=64,
                             # number of state-action pairs used to update discriminator per inner-iteration
    ...
    gan_gcl = GAN_GCL_Single(env, ppo_policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=expr_logger)
    gan_gcl.train()
