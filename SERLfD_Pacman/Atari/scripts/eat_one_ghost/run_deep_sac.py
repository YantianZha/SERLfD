...
"""
experiment_name = 'sac-car-racing'
wandb_project_name = 'car-racing'
...
def run_game():
    ...
    env = PytorchImage(GymRenderWrapper(gym.make('CarRacing-v0')), is_to_gray=True, is_normalize=False,
    ...
    hyper_params = Dict(dict(gamma=0.99,
                             tau=1e-3,
                             buffer_size=int(1e5),  # open-ai baselines: int(1e4)
                             batch_size=64,
                             init_random_actions=int(1e4),
    ...
    network_cfg = Dict(dict(use_cnn=True,
                            # MLP setting
                            fc_hidden_sizes_actor=[512],
                            fc_hidden_sizes_qf=[512],
    ...
    optim_cfg = Dict(dict(lr_actor=1e-4,
                          lr_qf=1e-4,
                          lr_entropy=1e-4,
                          eps_entropy=1e-8,
                          eps_qf=1e-8,
