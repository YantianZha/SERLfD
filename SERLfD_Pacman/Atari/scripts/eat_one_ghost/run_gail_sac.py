...
from learning_agents.irl.gail.gail import GAIL
...
experiment_name = 'gail-lunar-continuous'
...
def run_game():
    ...
    policy_hyper_params = Dict(dict(gamma=0.99,
                                    ...
                                    buffer_size=int(1e3),      # not used in irl algorithms
                                    ...
                                    num_iteration_update=5,  # multiple learning updates per iteration
                                    w_entropy=1e-3,
                                    w_mean_reg=0.0,
                                    w_std_reg=0.0,
    ...
    sac_policy = SACPolicy(env, policy_args, policy_log_cfg, policy_hyper_params,
    ...
    hyper_params = Dict(dict(buffer_size=int(1e3),  # size of (sampled/expert) trajectories buffer
                             ...
                             is_fusion=True,    # whether also use past samples to update irl (otherwise only use new sampled traj)
                             traj_batch_size=32,    # number of trajectories used to update discriminator per iteration (-1 means using all trajectories)
                             traj_fixed_length=15,  # the fixed length of trajectories
    ...
    optim_cfg = Dict(dict(lr_discrim=3e-4, weight_decay=1e-4))

    gail_agent = GAIL(env, sac_policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=expr_logger)
    gail_agent.train()
