...
def run_game():
    ...
    policy_hyper_params = Dict(dict(gamma=0.99,
                                    ...
                                    sample_size_iter=2048,
                                    # total sample size (state-action pairs) to collect before TRPO update (default: 2048)
                                    buffer_size=int(1e3),
                                    traj_batch_size=64,  # number of trajectories used to update controller_policy each iteration
    ...
    gail_agent = GAIL(env, trpo_policy, args, log_cfg, hyper_params, network_cfg, optim_cfg, encoder=None, logger=expr_logger)
    gail_agent.train()
