...
from algorithms.fd.se_sac_mlp_agent_v2 import Agent
...
def get(env, args):
    ...
    if hyper_params["STATES_WITH_UTILITIES"]:
        simple_states_dim = simple_states_dim + hyper_params["NUM_PREDICATES"]
    ...
    actor = TanhGaussianDistParams(
        input_size=simple_states_dim, output_size=action_dim, hidden_sizes=hidden_sizes_actor
    ).to(device)

    # create v_critic
    vf = MLP(input_size=simple_states_dim, output_size=1, hidden_sizes=hidden_sizes_vf).to(
        device
    )
    vf_target = MLP(
        input_size=simple_states_dim, output_size=1, hidden_sizes=hidden_sizes_vf
    ).to(device)
    vf_target.load_state_dict(vf.state_dict())

    # create q_critic
    qf_1 = FlattenMLP(
        input_size=simple_states_dim + action_dim, output_size=1, hidden_sizes=hidden_sizes_qf
    ).to(device)
    qf_2 = FlattenMLP(
        input_size=simple_states_dim + action_dim, output_size=1, hidden_sizes=hidden_sizes_qf
    ).to(device)
