...
from algorithms.fd.td3_agent_airl import Agent
...
def get(env, args):
    ...
    if not hyper_params["STATES_WITH_PREDICATES"]:
    ...
    # create critic1
    critic1 = MLP(
        input_size=simple_states_dim + action_dim,
        output_size=1,
    ...
    value_net_optim = optim.Adam(
            value_net.parameters(),
            lr=hyper_params["LR_EXPLAINER"],
            weight_decay=hyper_params["LAMBDA2"]
        )

    reward_net_optim = optim.Adam(
    ...
    models = (actor, actor_target, critic1, critic1_target, critic2, critic2_target, value_net, reward_net)
    optims = (actor_optim, critic_optim, value_net_optim, reward_net_optim)
