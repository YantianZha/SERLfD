...
import envs.fetch.fetch_step_acc as fetch
...
from kair_algorithms_draft.scripts.envs.fetch.fetch_step_acc import accurateIK, setMotors
...
class fetchGraspingProceduralEnvIG(BaseEnv):
  def __init__(
          self,
          config_file,
          scene_id=None,
    ...
    super(fetchGraspingProceduralEnvIG, self).__init__(config_file=config_file,
                                        scene_id=scene_id,
                                        mode=mode,
...
class fetchGraspingProceduralEnv(gym.Env):
  ...
  def __init__(
    ...
    self.s = Simulator(mode='iggui', image_width=960,
                  image_height=720, device_idx=0, rendering_settings=settings)
    self.s.import_ig_scene(scene)

    # ycb_object = YCBObject('025_mug')
    self.ycb_object = ArticulatedObject(
        '/home/yz/Research/venv/lib/python3.6/site-packages/iGibson/gibson2/data/assets/models/mugs/1eaf8db2dd2b710c7d5b1b70ae595e60/1eaf8db2dd2b710c7d5b1b70ae595e60.urdf', scale=0.15)
    self.s.import_object(self.ycb_object, use_pbr=False, use_pbr_mapping=False)


    conf_str, self.conf_data = loadYAML(
      "../config/fetch_serl_push_env_acc.yaml")
    self.get_params()
    print("Call get_obs")
    ...
    # else:
    self.a_min, self.a_max = np.array([-0.1, -0.1, -math.pi/6]), np.array(
        [0.1, 0.1, math.pi/6])
  ...
  def setup(self):
    ...
    self._urdf_list = [[local_env_data_path + '/../../models/insert_task_cube2_block4/model.urdf', [0.55, 0.1, 0.42]], [local_env_data_path + '/../../models/push_ico1/model.urdf', [0.55, -0.1, 0.42]]]

    logging.info('urdf_list %s', self._urdf_list)

    self.s.viewer.min_cam_z = 1.0
    self.s.viewer.px = -0.8
    self.s.viewer.py = 2.7
    self.s.viewer.pz = 1.08
    self.s.viewer.view_direction = np.array([0, 1, -0.5])
    self.s.viewer.reset_fn = lambda: reset_fn(ycb_object=self.ycb_object)

    # table_path = os.path.join(self._urdf_root, 'table_big.urdf')
    table_path = local_env_data_path + '/../../models/table/table_big.urdf'
    a = pybullet.loadURDF(
        table_path, basePosition=[1.0, 0.0, 0.], useFixedBase=True)
    textureId = pybullet.loadTexture(local_env_data_path + '/../../models/table/textures/wood.jpeg')
    pybullet.changeVisualShape(a, -1, textureUniqueId=textureId)

    l = np.random.choice([0, 1])
    self.Locs = [0.35, -0.1, 0.3651, 0.35, 0.1, 0.3651]
    BlockLocs = [self.Locs[:3], self.Locs[3:]] if l == 1 else [self.Locs[3:], self.Locs[:3]]

    block3_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block3/model.urdf', basePosition=BlockLocs[0], useFixedBase=True)

    block4_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block4/model.urdf', basePosition=BlockLocs[1], useFixedBase=True)

    # pybullet.loadURDF(
    #   'fetch_description/models/insert_task_cylinder2_block3/model.urdf', basePosition=[0.4, -0.1, 0.43],
    #   )
    # textureId = pybullet.loadTexture('/home/yz/Downloads/wood.jpeg')
    # pybullet.changeVisualShape(a, -1, textureUniqueId=textureId)


    self._block_uids = OrderedDict()
    self._block_uids['insert_task_block_1'] = block3_id
    self._block_uids['insert_task_block_2'] = block4_id
  ...
  def reset(self):
    ...
    self._fetch.reset(endeffector_pos=self.init_pos)#, fingerDist=self.gripper_min_dist)
    ...
    self.obs_targets_distance = np.linalg.norm(self.simple_states['cube1'][:2] - self.simple_states['insert_task_block_2'][:2]) + np.linalg.norm(self.simple_states['cylinder1'][:2] - self.simple_states['insert_task_block_1'][:2])
  ...
  def _step_continuous(self, action):
    ...
    self._fetch.move_pos(relative_pos=np.concatenate((action[:2], [0])), relative_global_euler=[0, 0, action[-1]], numSteps=1000)
  ...
  def _termination(self):
    ...
    for obj_name, region in self.target_region.items():
      if self.simple_states[obj_name][2] <= 0.35:
        print("A target fell down. Impossible to finish the task. Episode ends.")
        return True

    for d in self.done_predicates:
      if not self.simple_states['predicates'][d] == self.predicate_true_value:
        return False
    self.success = True
    return True
  ...
  def _reward(self):
    ...
    # print("CCC", self.init_target_distance)
    for d in self.done_predicates:
      if self.simple_states['predicates'][d] == self.predicate_true_value:
        finished += 1
  ...
  def mouse_control(self, grasp, back_step=1):
    ...
    targetOriRollId = pybullet.addUserDebugParameter("targetOriRoll", -3.15, 3.15, actualEndEffectorOrnEuler[0])
    targetOriPitchId = pybullet.addUserDebugParameter("targetOriPitch", -3.15, 3.15, actualEndEffectorOrnEuler[1])
    targetOriYawId = pybullet.addUserDebugParameter("targetOriYaw", actualEndEffectorOrnEuler[2]+self.a_min[2], actualEndEffectorOrnEuler[2]+self.a_max[2], actualEndEffectorOrnEuler[2])
