...
class fetchPush3DEnv(gym.Env):
  ...
  def __init__(
    ...
    conf_str, self.conf_data = loadYAML(
      "../config/fetch_serl_push3d_env_acc.yaml")
    self.get_params()
    print("Call get_obs")
    ...
    #     [0.05, 0.05, math.pi/2])
    self.a_min, self.a_max = np.array([-0.1, -0.1, -0.04, -math.pi/2]), np.array(
        [0.1, 0.1, 0.04, math.pi/2])
  ...
  def setup(self):
    ...
    self._urdf_list = [[local_env_data_path + '/../../models/insert_task_cube2_block4/model.urdf', [0.61, 0.1, 0.42]], [local_env_data_path + '/../../models/insert_task_cylinder2_block3/model_push.urdf', [0.61, -0.1, 0.42]]]
    ...
    self.Locs = [0.42, 0, 0.3651] #[0.42, -0.1, 0.3651, 0.42, 0.1, 0.3651]
    BlockLocs = [self.Locs]
    ...
    _ = pybullet.loadURDF(
      ...
      basePosition=[self._urdf_list[1][1][0], self._urdf_list[1][1][1], config[1][2]+0.015],
      physicsClientId=self.cid)
  ...
  def _get_current_predicate_values(self):
    ...
    def not_opened(entity):
      ...
      if contacts:
        for p in contacts:
          if p[5][2] >= height-0.002: # If the z pose value is >= the entity's height
    ...
    def relative_orn(entity, target, expected, predicate): # L, R, F, B # TODO Up, Down, Overloapping
      ...
      if 'initial' in predicate and self._env_step > 0:
        predicate_id = self.predicates_list.index(predicate)
        return self.previous_predicates_values[predicate_id]

      possibilities = {'left': 0, 'right': 1, 'front': 2, 'back': 3}
      expected = possibilities[expected]
      left = True
      front = False
      if abs(self.simple_states[entity][0]) < abs(self.simple_states[target][0]):
        front = True
      if self.simple_states[entity][1] > self.simple_states[target][1]:
        left = False
