...
class fetchGraspingProceduralEnv(gym.Env):
  ...
  def __init__(
    ...
    conf_str, self.conf_data = loadYAML(
      "../config/fetch_serl_push_env.yaml")
    self.get_params()
    print("Call get_obs")
    ...
    # else:
    self.a_min, self.a_max = np.array([-0.2, -0.2]), np.array(
        [0.2, 0.2])
  ...
  def setup(self):
    ...
    block3_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block3/model.urdf', basePosition=[0.35, -0.1, 0.3651], useFixedBase=True,
      physicsClientId=self.cid)

    block4_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block4/model.urdf', basePosition=[0.35, 0.1, 0.3651], useFixedBase=True,
      physicsClientId=self.cid)
  ...
  def _step_continuous(self, action):
    ...
    self._env_step += 1
    self._fetch.applyAction(np.concatenate((action, [0.0, 0.0, 0.0])))
  ...
  def _reward(self):
    ...
    if self.use_shaping and self.obs_targets_distance + neg_dist > 0:
      ...
      return shape_rwd + self.fail_reward
    else:
