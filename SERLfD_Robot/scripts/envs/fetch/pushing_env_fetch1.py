...
import envs.fetch.fetch_step as fetch
...
class fetchGraspingProceduralEnv(gym.Env):
  ...
  def __init__(
    ...
    conf_str, self.conf_data = loadYAML(
      os.getcwd() + "/../config/fetch_serl_push_env.yaml")
    self.get_params()
    print("Call get_obs")
    ...
    # else:
    self.a_min, self.a_max = np.array([-0.2, -0.2, -0.2, -1.57]), np.array(
        [0.2, 0.2, 0.2, 1.57])
  ...
  def setup(self):
    ...
    """
    test = self._test
    self._urdf_list = [[local_env_data_path + '/../../models/insert_task_cube2_block4/model.urdf', [0.6, 0.1, 0.62]], [local_env_data_path + '/../../models/insert_task_cylinder2_block3/model.urdf', [0.6, -0.1, 0.62]]]
    ...
    a = pybullet.loadURDF(
        table_path, basePosition=[1.0, 0.0, 0.2], useFixedBase=True,
    ...
    block3_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block3/model.urdf', basePosition=[0.35, -0.1, 0.566], useFixedBase=True,
      physicsClientId=self.cid)

    block4_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block4/model.urdf', basePosition=[0.35, 0.1, 0.566], useFixedBase=True,
      physicsClientId=self.cid)
  ...
  def _get_current_predicate_values(self):
    ...
    def pushed(entity):
      ...
      if np.allclose(a=curr_relative_pose, b=self.prev_relative_pose[entity],
                     atol=0.4) and not np.allclose(a=self.simple_states_prev[entity], b=self.simple_states[entity], atol=0.05) :
    ...
    return dict(zip(self.predicates_list, predicates_values))
  ...
  def _get_simple_observation(self):
    ...
    self.curr_simple_states = np.concatenate((pos_and_ori, end_effector_pos, end_effector_ori, list(predicates.values())))
  ...
  def _reward(self):
    ...
    if finished > self.finished_prev:
      ...
      return -distance.euclidean(self.simple_states['cube1'][:2], self.simple_states['insert_task_block_2'][:2])-distance.euclidean(self.simple_states['cylinder1'][:2], self.simple_states['insert_task_block_1'][:2])
