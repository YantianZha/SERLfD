...
class fetchPush3DEnv(gym.Env):
  ...
  def __init__(
    ...
    conf_str, self.conf_data = loadYAML(
      "../config/fetch_serl_push3d_env_acc_3.yaml")
    self.get_params()
    print("Call get_obs")
  ...
  def get_params(self):
    ...
    self.predicates_groups = self.conf_data['env']['predicates_groups']
  ...
  def _get_current_predicate_values(self):
    ...
    not_used_objects = {'cube1', 'cylinder1'} - set(self._used_objs)
    for obj in not_used_objects:
      for i in self.predicates_groups[obj]:
        predicates_values[i] = 0.
  ...
  def _get_simple_observation(self):
    ...
    obj = [1, -1] if 'cube1' in self._used_objs else [-1, 1]

    self.curr_simple_states = np.concatenate((obj, self.simple_states['locs'], pos_and_ori, end_effector_pos, end_effector_ori, list(predicates.values())))
