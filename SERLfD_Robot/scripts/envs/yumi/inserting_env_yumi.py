...
local_env_data_path = 'envs/yumi/yumi_description'
# OSS_DATA_ROOT = os.getcwd() + '/yumi_simple_simulation/yumi_simple_description/models'

# pylint: enable=bad-import-order
# pylint: enable=g-import-not-at-top


class yumiInsertingEnv(gym.Env):
  ...
  def __init__(
    ...
    conf_str, self.conf_data = loadYAML(
      os.getcwd() + "/../config/yumi_serl_insert_env.yaml")
    self.get_params()
    print("Call get_obs")
    ...
    self.gripper_min_dist = self.conf_data['yumi']['gripper_min_dist']
    self.gripper_max_dist = self.conf_data['yumi']['gripper_max_dist']
    self.exe_single_group = self.conf_data['yumi']['exe_single_group']
    # if self.exe_single_group:
    #   self.a_min, self.a_max = np.array([0., 0., 0.25, -0.4, 0.37, -2, -1]), np.array(
    #     [1., 1., 0.75, 0.4, 0.91, 2, 1])
    # else:
    self.a_min, self.a_max = np.array([-0.2, -0.2, -0.2, -0.4, -1]), np.array(
        [0.2, 0.2, 0.2, 0.4, 1])
  ...
  def get_params(self):
    self.group_name = self.conf_data['yumi']['group_name']
    self.n_actions = self.conf_data['yumi']['n_actions']
    self.n_max_iterations = self.conf_data['yumi']['max_iterations']
    self.init_pos = self.conf_data['yumi']['init_pos']
    self.n_observations = int(self.conf_data['yumi']['n_observations'])
    self.gripper_max_effort = float(self.conf_data['yumi']['gripper_max_effort'])
  ...
  def setup(self):
    ...
    """
    test = self._test
    self._urdf_list = [[local_env_data_path + '/../../models/insert_task_cube2_block4/model.urdf', [0.4, 0.1, 0.05]], [local_env_data_path + '/../../models/insert_task_cylinder2_block3/model.urdf', [0.4, -0.1, 0.05]]]
    ...
    block3_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block3/model.urdf', basePosition=[0.55, -0.1, 0.05], useFixedBase=True,
      physicsClientId=self.cid)

    block4_id = pybullet.loadURDF(
      local_env_data_path + '/../../models/insert_task_block4/model.urdf', basePosition=[0.55, 0.1, 0.05], useFixedBase=True,
      physicsClientId=self.cid)

    # pybullet.loadURDF(
    #   'yumi_description/models/insert_task_cylinder2_block3/model.urdf', basePosition=[0.4, -0.1, 0.43],
    #   physicsClientId=self.cid)
    # textureId = pybullet.loadTexture('/home/yz/Downloads/wood.jpeg', physicsClientId=self.cid)
    # pybullet.changeVisualShape(a, -1, textureUniqueId=textureId, physicsClientId=self.cid)


    self._block_uids = {'insert_task_block_1': block3_id, 'insert_task_block_2': block4_id}
  ...
  def reset(self):
    ...
    look = [0.4, 0.3, 0.84]
    distance = 1.
    pitch = -110 #+ self._cam_random * np.random.uniform(-3, 3)
    ...
    self._proj_matrix = pybullet.computeProjectionMatrixFOV(
        fov, aspect, near, far)
    self._env_step = 0
    self.predicates_values = dict(zip(self.predicates_list, [-1.] * len(self.predicates_list)))
    ...
    self._yumi.applyAction([0.5, 0, 0.5, 0, 1])
    for i in range(200):
      pybullet.stepSimulation(physicsClientId=self.cid)
  ...
  def _get_image_observation(self):
    ...
    if self.merge_simple_states_to_img:
      ...
      curr_simple_states = self._get_simple_observation()
  ...
  def _get_current_predicate_values(self):
    ...
    def grasped(entity):
      ...
      if np.allclose(a=curr_relative_pose, b=self.prev_relative_pose[entity],
                     atol=0.02) and obj_ee_dist < 0.2 and sum(self.gripper_joint_pose) < 0.04:
    ...
    self.predicates_values = dict(zip(self.predicates_list, predicates_values))
    return predicates_values
  ...
  def _get_simple_observation(self):
    ...
    state = pybullet.getLinkState(
        self._yumi.yumiUid, self._yumi.yumiEndEffectorIndex,
        physicsClientId=self.cid)
  ...
  def _grasping(self):
    ...
    for _ in range(1000):
      ...
      self._yumi.applyDeltaAction(grasp_action)
      pybullet.stepSimulation(physicsClientId=self.cid)
  ...
  def _step_continuous(self, action):
    ...
    self._env_step += 1
    self._yumi.applyDeltaAction(np.concatenate((action[:4], [0])))
    ...
    # state = pybullet.getLinkState(self._yumi.yumiUid,
    #                               self._yumi.yumiEndEffectorIndex,
    #                               physicsClientId=self.cid)
    ...
    #     self._yumi.applyAction(grasp_action)
    #     pybullet.stepSimulation(physicsClientId=self.cid)
    #     finger_angle -= 0.3/100.
    #     if finger_angle < 0:
    #       finger_angle = 0
  ...
  def _reward(self):
    def get_continuous_rwd_shapping():
      ...
      if 1.0 not in predicate_value:
        shaping_rewards = max(
          -np.linalg.norm(simple_states['ee'][:3] - simple_states['cube1'][:3]),
          -np.linalg.norm(simple_states['ee'][:3] - simple_states['cylinder1'][:3]))
          ...
          if self.predicates_list[np.where(predicate_value == 1.0)[0][0]] == st[0]:
            if id == 0:
              shaping_rewards = -np.linalg.norm(
                simple_states['cube1'][:3] - simple_states['insert_task_block_2'][:3])
            else:
              shaping_rewards = -np.linalg.norm(
                simple_states['cylinder1'][:3] - simple_states['insert_task_block_1'][:3])

      elif np.where(predicate_value == 1.0)[0][-1] == 2 or np.where(predicate_value == 1.0)[0][-1] == 4:
        shaping_rewards = -np.linalg.norm(
          simple_states['ee'][:3] - simple_states['cube1'][:3])

      elif np.where(predicate_value == 1.0)[0][-1] == 3 or np.where(predicate_value == 1.0)[0][-1] == 5:
        shaping_rewards = -np.linalg.norm(
          simple_states['ee'][:3] - simple_states['cylinder1'][:3])  # care about grasping cylinder
