...
class yumiReachingEnv(gym.Env):
  ...
  def __init__(
    ...
    # else:
    self.a_min, self.a_max = np.array([-0.3, -0.3, -0.3, -0.4, -1]), np.array(
        [0.3, 0.3, 0.3, 0.4, 1])
  ...
  def _update_simple_states(self):
    ...
    self.robot_base_state_now = self.get_target_objects_state(['yumi'])['yumi']  # self.update_base()
    ...
    self.obj_robot_states_now = self.get_target_objects_state(self.target_region.keys(), 'yumi')
  ...
  def _termination(self):
    ...
    d = np.linalg.norm(simple_states[:3] - simple_states[14:17])
    # print("DD 1: ", d)
    if d < 0.3 or self._env_step >= self._max_episode_steps:
      return True
    else:
      return False
  ...
  def _reward(self):
    reward = 0
    simple_states = self.curr_simple_states
    d = np.linalg.norm(simple_states[:3] - simple_states[12:15])
    print("DD 2: ", d)
    if d < 0.3:
