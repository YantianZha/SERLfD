...
local_env_data_path = '/home/yz/Research/kair_algorithms_draft/scripts/envs/fetch/fetch_description'
...
class fetchGraspingProceduralEnv(gym.Env):
  ...
  def __init__(
      ...
      render_width=640,
      render_height=480,
      downsample_width=120,
      downsample_height=90,
      test=False):
    ...
    conf_str, self.conf_data = loadYAML(
      "/home/yz/Research/kair_algorithms_draft/config/fetch_serl_push_env_acc.yaml")
    self.get_params()
    print("Call get_obs")
  ...
  def setup(self):
    ...
    self._urdf_list = [[local_env_data_path + '/../../models/insert_task_cube2_block4/model.urdf', [0.61, 0.1, 0.42]], [local_env_data_path + '/../../models/push_ico1/model.urdf', [0.61, -0.1, 0.42]]]
    ...
    self.Locs = [0.42, -0.1, 0.3651, 0.42, 0.1, 0.3651]
  ...
  def _step_continuous(self, action):
    ...
    self._fetch.move_pos(relative_pos=np.concatenate((action[:2], [0])), absolute_global_euler=[self.init_pos[3], self.init_pos[4], action[-1]], numSteps=100, gripper_target_pos=0.)
